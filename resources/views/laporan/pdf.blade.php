...
<div class="kop">
    ...
    <div class="kop-stamp">
        <div class="stamp-box">
            ...
            <div class="stamp-date">Dicetak: {{ now()->isoFormat('D MMM Y') }}</div>
        </div>
    </div>
</div>
...
<div class="report-title">
    ...
    <p>Periode: {{ \Carbon\Carbon::parse($dari)->isoFormat('D MMMM Y') }} — {{ \Carbon\Carbon::parse($sampai)->isoFormat('D MMMM Y') }}</p>
</div>

<!-- Rekap Stats -->
<div class="rekap-row">
    <div class="rekap-cell"><div class="rekap-box r-hadir"><div class="num">{{ $rekap['hadir'] }}</div><div class="lbl">Hadir</div></div></div>
    <div class="rekap-cell"><div class="rekap-box r-telat"><div class="num">{{ $rekap['telat'] }}</div><div class="lbl">Terlambat</div></div></div>
    <div class="rekap-cell"><div class="rekap-box r-absen"><div class="num">{{ $rekap['tidak_hadir'] }}</div><div class="lbl">Tidak Hadir</div></div></div>
    <div class="rekap-cell"><div class="rekap-box r-izin"><div class="num">{{ $rekap['izin'] }}</div><div class="lbl">Izin</div></div></div>
    <div class="rekap-cell"><div class="rekap-box r-sakit"><div class="num">{{ $rekap['sakit'] }}</div><div class="lbl">Sakit</div></div></div>
    <div class="rekap-cell"><div class="rekap-box r-alpha"><div class="num">{{ $rekap['alpha'] }}</div><div class="lbl">Alpha</div></div></div>
</div>
...
<table>
    ...
    <tr>
        ...
        <td style="text-align:center">{{ $p->tanggal->isoFormat('ddd, D/M/Y') }}</td>
        <td><strong>{{ $p->guru->nama ?? '-' }}</strong></td>
        <td style="font-size:7.5px;color:#666">{{ $p->guru->id_pengguna ?? '-' }}</td>
        ...
        <td style="text-align:center">
            ...
            <span class="s s-{{ $sc }}">{{ strtoupper(str_replace('_',' ',$p->status)) }}</span>
        </td>
        <td style="text-align:center">
            @if($p->menit_telat > 0)
                <strong style="color:#F57F17">+{{ $p->menit_telat }}</strong>
            @else -
            @endif
        </td>
        <td style="text-align:center;font-size:7.5px">{{ ucfirst($p->metode ?? '-') }}</td>
    </tr>
    @empty
...
<div class="pdf-footer">
    <div class="footer-left">
        Dicetak: {{ now()->isoFormat('dddd, D MMMM Y, HH:mm') }} WIB<br>
        Total Data: {{ $presensis->count() }} record<br>
